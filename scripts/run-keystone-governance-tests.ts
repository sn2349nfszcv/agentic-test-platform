...
// Keystone-Governance platform test execution script

import { config } from 'dotenv';
import { resolve } from 'path';
import { mkdirSync, existsSync } from 'fs';
import { TestOrchestrator } from '../lib/core/TestOrchestrator';
import { KeystoneGovernanceBetaUserAgent } from '../lib/agents/keystone-governance/BetaUserAgent';
...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║   AGENTIC TEST PLATFORM - Keystone-Governance           ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  Platform: Keystone-Governance\n`);
  ...
  const baseUrl = process.env.KEYSTONE_BASE_URL || 'http://localhost:3000';
  ...
  try {
    const result = await orchestrator.executeTest(
      ...
      KeystoneGovernanceBetaUserAgent
    );
    ...
    const reportPath = resolve(resultsDir, `keystone-governance-test-report-${timestamp}.md`);
