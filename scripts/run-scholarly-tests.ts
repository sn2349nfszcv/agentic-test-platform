...
// Scholarly-AI platform test execution script

import { config } from 'dotenv';
import { resolve } from 'path';
import { mkdirSync, existsSync } from 'fs';
import { TestOrchestrator } from '../lib/core/TestOrchestrator';
import { ScholarlyBetaUserAgent } from '../lib/agents/scholarly/BetaUserAgent';
...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║   AGENTIC TEST PLATFORM - Scholarly-AI Platform          ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  Platform: Scholarly-AI\n`);
  ...
  const scholarlyBaseUrl = process.env.SCHOLARLY_BASE_URL || 'http://localhost:3002';
  console.log(`  Target URL: ${scholarlyBaseUrl}\n`);
  ...
  try {
    ...
    const reportPath = resolve(resultsDir, `scholarly-test-report-${timestamp}.md`);
