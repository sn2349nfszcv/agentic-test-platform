...
// AHPRA-Pro platform test execution script

import { config } from 'dotenv';
import { resolve } from 'path';
import { mkdirSync, existsSync } from 'fs';
import { TestOrchestrator } from '../lib/core/TestOrchestrator';
import { AHPRAProBetaUserAgent } from '../lib/agents/ahpra-pro/BetaUserAgent';
...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║   AGENTIC TEST PLATFORM - AHPRA-Pro                     ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  Platform: AHPRA-Pro\n`);
  ...
  const baseUrl = process.env.AHPRA_BASE_URL || 'http://localhost:3000';
  ...
  const testConfig: TestConfig = {
    platform: 'ahpra-pro',
    baseUrl,
    apiKey: process.env.AHPRA_API_KEY,
  ...
  try {
    ...
    const reportPath = resolve(resultsDir, `ahpra-pro-test-report-${timestamp}.md`);
