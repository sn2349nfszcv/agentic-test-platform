...
// ReadWorthy platform test execution script

import { config } from 'dotenv';
import { resolve } from 'path';
import { mkdirSync, existsSync } from 'fs';
import { TestOrchestrator } from '../lib/core/TestOrchestrator';
import { ReadWorthyBetaUserAgent } from '../lib/agents/readworthy/BetaUserAgent';
...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║   AGENTIC TEST PLATFORM - ReadWorthy Platform            ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  Platform: ReadWorthy\n`);
  ...
  const readworthyBaseUrl = process.env.READWORTHY_BASE_URL || 'http://localhost:3001';
  console.log(`  Target URL: ${readworthyBaseUrl}\n`);
  ...
  const testConfig: TestConfig = {
    platform: 'readworthy',
    baseUrl: readworthyBaseUrl,
    apiKey: process.env.READWORTHY_API_KEY,
  ...
  try {
    ...
    const reportPath = resolve(resultsDir, `readworthy-test-report-${timestamp}.md`);
