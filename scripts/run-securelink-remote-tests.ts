...
// SecureLink-Remote platform test execution script

import { config } from 'dotenv';
import { resolve } from 'path';
import { mkdirSync, existsSync } from 'fs';
import { TestOrchestrator } from '../lib/core/TestOrchestrator';
import { SecureLinkRemoteBetaUserAgent } from '../lib/agents/securelink-remote/BetaUserAgent';
...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║   AGENTIC TEST PLATFORM - SecureLink-Remote             ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  Platform: SecureLink-Remote\n`);
  ...
  const baseUrl = process.env.SECURELINK_BASE_URL || 'http://localhost:3000';
  ...
  const testConfig: TestConfig = {
    platform: 'securelink-remote',
    baseUrl,
    apiKey: process.env.SECURELINK_API_KEY,
  ...
  try {
    ...
    const reportPath = resolve(resultsDir, `securelink-remote-test-report-${timestamp}.md`);
