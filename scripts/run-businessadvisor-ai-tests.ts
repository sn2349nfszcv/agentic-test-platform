...
// BusinessAdvisor-AI platform test execution script

import { config } from 'dotenv';
import { resolve } from 'path';
import { mkdirSync, existsSync } from 'fs';
import { TestOrchestrator } from '../lib/core/TestOrchestrator';
import { BusinessAdvisorAIBetaUserAgent } from '../lib/agents/businessadvisor-ai/BetaUserAgent';
...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║   AGENTIC TEST PLATFORM - BusinessAdvisor-AI            ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  Platform: BusinessAdvisor-AI\n`);
  ...
  const baseUrl = process.env.BUSINESSADVISOR_BASE_URL || 'http://localhost:3000';
  ...
  const testConfig: TestConfig = {
    platform: 'businessadvisor-ai',
    baseUrl,
    apiKey: process.env.BUSINESSADVISOR_API_KEY,
  ...
  try {
    const result = await orchestrator.executeTest(
      ...
      BusinessAdvisorAIBetaUserAgent
    );
    ...
    const reportPath = resolve(resultsDir, `businessadvisor-ai-test-report-${timestamp}.md`);
