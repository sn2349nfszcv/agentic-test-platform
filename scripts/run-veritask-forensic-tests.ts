...
// Veritask-Forensic platform test execution script

import { config } from 'dotenv';
import { resolve } from 'path';
import { mkdirSync, existsSync } from 'fs';
import { TestOrchestrator } from '../lib/core/TestOrchestrator';
import { VeritaskForensicBetaUserAgent } from '../lib/agents/veritask-forensic/BetaUserAgent';
...
async function main() {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  console.log('║   AGENTIC TEST PLATFORM - Veritask-Forensic             ║');
  console.log('╚═══════════════════════════════════════════════════════════╝\n');
  ...
  console.log(`  Platform: Veritask-Forensic\n`);
  ...
  const baseUrl = process.env.VERITASK_BASE_URL || 'http://localhost:3000';
  ...
  const testConfig: TestConfig = {
    platform: 'veritask-forensic',
    baseUrl,
    apiKey: process.env.VERITASK_API_KEY,
  ...
  try {
    ...
    const reportPath = resolve(resultsDir, `veritask-forensic-test-report-${timestamp}.md`);
